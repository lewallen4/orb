...
class OrbClient:
    def __init__(self, root):
        ...
        self.root.geometry("639x1000")
        self.root.minsize(639, 800)
        ...
        # Avatar data
        self.avatar_data = {'body': 1, 'top': 1, 'head': 1}
        self.avatars = {}
    ...
    def setup_ui(self):
        ...
        self.chamber_frame.pack(expand=True, fill=tk.BOTH, pady=10)
        
        self.chamber_canvas = tk.Canvas(self.chamber_frame, width=600, height=300, bg='#000010')
        self.chamber_canvas.pack(expand=True, fill=tk.BOTH, padx=5, pady=5)        
        
        # Connection details
        detail_frame = tk.LabelFrame(self.content_frame, text=" CONNECTION DETAILS ", 
                                   font=(self.custom_font, 10), bg=self.BG_COLOR, 
                                   fg=self.FG_COLOR, labelanchor='n')
        ...
        # Device selection
        dev_frame = tk.LabelFrame(self.content_frame, text=" AUDIO CONFIGURATION ", 
                                font=(self.custom_font, 10), bg=self.BG_COLOR, 
                                fg=self.FG_COLOR, labelanchor='n')
        ...
        # Participants list
        part_frame = tk.LabelFrame(self.content_frame, text=" PARTICIPANTS ", 
                                 font=(self.custom_font, 10), bg=self.BG_COLOR, 
                                 fg=self.FG_COLOR, labelanchor='n')
        ...
        creator_frame = tk.LabelFrame(self.content_frame, text="HOLOGRAM APPEARANCE", 
                                    fg="#00ffff", bg=self.BG_COLOR)
        creator_frame.pack(pady=10, fill=tk.X)
        
        # Body Type
        tk.Label(creator_frame, text="Chassis:", fg="white", bg=self.BG_COLOR).grid(row=0, column=0)
        self.body_var = tk.IntVar(value=1)
        ttk.Combobox(creator_frame, values=[1, 2], textvariable=self.body_var,
                    state="readonly").grid(row=0, column=1)
        self.body_var.trace('w', lambda *args: self.update_preview())
        
        # Garment
        tk.Label(creator_frame, text="Vestment:", fg="white", bg=self.BG_COLOR).grid(row=1, column=0)
        self.top_var = tk.IntVar(value=1)
        ttk.Combobox(creator_frame, values=[1, 2, 3], textvariable=self.top_var,
                    state="readonly").grid(row=1, column=1)
        self.top_var.trace('w', lambda *args: self.update_preview())
        
        # Headwear
        tk.Label(creator_frame, text="Cowl:", fg="white", bg=self.BG_COLOR).grid(row=2, column=0)
        self.head_var = tk.IntVar(value=1)
        ttk.Combobox(creator_frame, values=[1, 2, 3], textvariable=self.head_var,
                    state="readonly").grid(row=2, column=1)
        self.head_var.trace('w', lambda *args: self.update_preview())
        
        # Preview
        self.preview_canvas = tk.Canvas(creator_frame, width=200, height=300, bg='#000010')
    ...
    def generate_avatar_image(self, data):
        try:
            base = Image.open(f"assets/base_{data['body']}.png").convert("RGBA")
            top = Image.open(f"assets/top_{data['top']}.png").convert("RGBA")
            head = Image.open(f"assets/head_{data['head']}.png").convert("RGBA")
    ...
    def update_chamber_view(self, avatars):
        ...
        self.chamber_canvas.delete("all")
        ...
        canvas_width = self.chamber_canvas.winfo_width()
        canvas_height = self.chamber_canvas.winfo_height()
        num_avatars = len(other_avatars)

        # Show message if no other participants
        if num_avatars == 0:
            self.chamber_canvas.create_text(
        ...
        for i, (addr, data) in enumerate(other_avatars.items()):
            ...
            try:
                ...
                if img:
                    ...
                    self.chamber_canvas.create_image(
                        x, y,
                        image=scaled_img,
                        anchor=tk.CENTER
                    )
    ...
    def update_loop(self):
        ...
        while self.running:
            try:
                # Update participant list - Simplified version without prefixes
                self.participants_list.delete(0, tk.END)
                
                # Show host name if available
                if self.host_name:
                    self.participants_list.insert(tk.END, self.host_name)
                ...
                self.participants_list.insert(tk.END, self.client_name)
